...
function fail(message: string): never {
  console.error(`NNACT Pro key verification failed: ${message}`);
  process.exit(1);
}

const publicKeyPath = values["public-key"] ?? process.env.NNPLICENSE_PUBLIC_KEY_PATH ?? DEFAULT_PUBLIC_KEY;
const resolvedPublicKeyPath = resolve(publicKeyPath);
if (!existsSync(resolvedPublicKeyPath)) {
  fail(`public key not found: ${resolvedPublicKeyPath}; provide --public-key or NNPLICENSE_PUBLIC_KEY_PATH`);
}

let token = values.license ?? process.env.NNPLICENSE_KEY;
...
if (!token) fail("provide --license, --license-file, or NNPLICENSE_KEY");
