...
export const businessSettingsSchema = z.object({
  ...
  numbering: z.object({
    ...
    expensePrefix: z.string().trim().min(1).max(16).regex(/^[A-Za-z0-9-]+$/).optional().default("NNACT/EXP"),
    expenseNextNumber: z.number().int().min(1).max(999_999_999).optional().default(1000),
    billPrefix: z.string().trim().min(1).max(16).regex(/^[A-Za-z0-9-]+$/).optional().default("NNACT/BILL"),
    billNextNumber: z.number().int().min(1).max(999_999_999).optional().default(1000),
    advancePrefix: z.string().trim().min(1).max(16).regex(/^[A-Za-z0-9-]+$/).optional().default("NNACT/ADV"),
